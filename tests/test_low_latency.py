...
import deep_ep
from utils import init_dist, bench, bench_kineto, calc_diff, hash_tensor, per_token_cast_back, per_token_cast_to_fp8_pertok
    ...
    for current_x in x_list:
        for return_recv_hook in (False, True):
            for dispatch_use_fp8 in (False, True):
                for per_token in (False, True) if dispatch_use_fp8 else (False, ):
                    for round_scale in (False, True) if (dispatch_use_fp8 and not per_token) else (False, ):
                        for use_ue8m0 in (False, True) if round_scale else (False, ):
                            if shrink_test and simulate_failure_and_skip(rank, "dispatch", expected_masked_ranks):
                                break
                            num_times += 1
                            num_scale_groups = 1 if per_token else hidden // 128
                            ...
                            simulated_gemm_x = per_token_cast_back(packed_recv_x[0].view(-1, hidden), packed_recv_x[1].view(-1, num_scale_groups)).view(packed_recv_x[0].shape) \
                            ...
                            for i in range(num_local_experts if do_check else 0):
                                ...
                                if current_x is x:
                                    ...
                                    if round_scale or per_token:
                                        assert calc_diff(recv_x[:, -1], recv_src_info.view(-1)) < (0.02 if per_token else 0.007)
                                    else:
                                    ...
                                    for j in range(num_ranks):
                                        ...
                                        if not round_scale and not per_token:
                            ...
                            for zero_copy in (False, ) if use_logfmt else (False, True):
                                ...
                                if do_check:
                                    ...
                                    if not round_scale and not per_token:
                                        ...
                                        assert diff < 5e-3, f'Error (per_token): {diff=}, {zero_copy=}'
